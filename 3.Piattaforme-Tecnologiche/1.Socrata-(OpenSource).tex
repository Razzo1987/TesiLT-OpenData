...
L’architettura del sistema può quindi essere suddivisa in una serie di componenti separate che la compongono come mostrato in figura \ref{fig:socrata-architecture}.
...
\item[\textcolor{Orange}{\ding{203}}] La richiesta viene trasformata in una serie di operazioni più elementari chiamante “mutations” e vengono passate al Data Coordinator in modo da essere eseguite sul database (Postgres).
...
Ciò permette in caso di guasti di re-sincronizzare il database.
\end{enumerate}
...
La Community Edition condivide lo stesso core della versione proprietaria, ma al momento manca completamente il front-end per gestire comodamente i dataset che è invece presente nella versione SaaS.
