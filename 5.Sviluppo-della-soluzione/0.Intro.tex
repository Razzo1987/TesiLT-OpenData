Per prima cosa  stata installata su macchina virtuale il sistema operativo \href{http://releases.ubuntu.com/precise/ubuntu-12.04.2-desktop-amd64.iso}{\texttt{Ubuntu 12.04 a 64 bit}}, necessario per poter installare CKAN dai pacchetti.\\
Da terminale sono stati installati i pacchetti richiesti e il pacchetto di CKAN versione 2.0:
...
Si  scelto di utilizzare un unica istanza di CKAN in questa macchina virtuale e quindi un solo file di configurazione dello schema Solr. Si  quindi installato il Java Development Kit e modificato il file di configurazione Jetty:
...
Si  poi configurato il database PostgreSQL creando lÕutente \texttt{ckanuser} a cui  stata data la proprietˆ del database \texttt{ckan\_default}. Successivamente si  modificato il valore di \texttt{sqlalchemy.url} nel file di configurazione \texttt{/etc/ckan/default/production.ini}. Infine si  inizializzato il database:
...
Si  proseguiti con lÕimpostazione del \textbf{DataStore}, un componente che fornisce un database per l'archiviazione strutturata di dati insieme a delle potenti API accessibili dal WEB. Tutto ci˜  perfettamente integrato nell'interfaccia web e nella gestione dei permessi di CKAN.\\
Per fare ci˜ si  abilitato lÕestensione, separatamente, nel file di configurazione:

\shellcmd{sudo gedit /etc/ckan/default/production.ini}
{\indent\indent\texttt{\footnotesize\textcolor{Orchid}{84:} 84: ckan.plugins = datastore stats json\_preview recline\_preview}}\\
DopodichŽ si  creato lÕutente \texttt{ckanreaduser} con sola lettura al database \texttt{datastore\_default} e si  assegnati i relativi permessi. Inoltre si  modificato il file di configurazione con gli opportuni valori:
...
Successivamente si sono impostati anche i componenti aggiuntivi \textbf{FileStore} e \textbf{File Uploads}, che contrariamente al FileStore, che fornisce un'archiviazione 'blob' di interi file senza alcun accesso o interrogazione a parti di essi, fanno in modo che CKAN si comporti come un database in cui i singoli elementi del dato sono accessibili e interrogabili.
...
Si  creata la directory per salvataggio in locale e si  modificato il file di configurazione per impostarne il path. Successivamente si  assegnati i permessi a CKAN sulla directory e infine si  impostato lÕurl e riavviato il server web:
...
Infine si  installato il \textbf{DataStorer}, che permette aggiunge automaticamente al DataStore i dati che vengono inseriti in CKAN (linkati o caricati nel FileStore). Ci˜ viene fatto attraverso un attivitˆ di analisi automatica e successiva aggiunta nel DataStore da un processo che viene eseguito in modo asincrono.\\
Si  quindi installato git, si  poi recuperato i sorgenti del pacchetto e installati i requisiti:

\shellcmd{sudo apt-get install git-core}
\shellcmd{sudo -s}
{\indent\texttt{\footnotesize\textcolor{NavyBlue}{\textasciitilde{}\#} . /usr/lib/ckan/default/bin/activate}}\\
{\indent\texttt{\footnotesize\textcolor{NavyBlue}{(default) \textasciitilde{}\#} pip install -e git+git://github.com/okfn/ckanext-datastorer
.git\#egg=ckanext-datastorer}}\\
{\indent\texttt{\footnotesize\textcolor{NavyBlue}{(default) \textasciitilde{}\#} pip install -r /usr/lib/ckan/default/src/ckanext-datastorer
/pip-requirements.txt}}\\
{\indent\texttt{\footnotesize\textcolor{NavyBlue}{(default) \textasciitilde{}\#} exit}}\\
